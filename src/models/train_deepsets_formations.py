# src/models/train_deepsets_formations.py
...
from src.data.play_features import FormationDataset, compute_formation_targets  # noqa: F401
...
def only_side(side_value: str):
    """Filter rows to a given player_side ('offense' or 'defense')."""
    import polars as pl
    def _sel(df: pl.DataFrame) -> pl.DataFrame:
        return df.filter(pl.col("player_side") == side_value)
    return _sel

def last_k_frames_per_team(k: int = 1):
    """Keep the last k frames per (game_id, play_id, player_side)."""
    import polars as pl
    def _sel(df: pl.DataFrame) -> pl.DataFrame:
        last = (
            df.group_by(["game_id", "play_id", "player_side"])
              .agg(pl.col("frame_id").max().alias("_last"))
        )
        df2 = df.join(last, on=["game_id", "play_id", "player_side"], how="left")
        return df2.filter(pl.col("frame_id") >= pl.col("_last") - (k - 1)).drop("_last")
    return _sel
...
def export_embeddings(
    parquet_path: Path,
    side: str,
    weights_path: Path,           # <— keep name consistent
    out_parquet: Path,
    n_max: int = 11,
    batch_size: int = 256,
):
